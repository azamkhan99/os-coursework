...
private:
    ...
    PageDescriptor *split_block(PageDescriptor **block_pointer, int source_order)
    {
        ...
        PageDescriptor *right_buddy = *block_pointer + pages_per_block(source_order-1);

        remove_block(left_buddy, source_order);
        insert_block(left_buddy, source_order-1);
        insert_block(right_buddy, source_order-1);

        assert(is_correct_alignment_for_order(left_buddy, source_order-1));
        assert(is_correct_alignment_for_order(right_buddy, source_order-1));
    ...
    PageDescriptor **merge_block(PageDescriptor **block_pointer, int source_order)
    {
        ...
        PageDescriptor *left_buddy = *block_pointer;
        PageDescriptor *right_buddy = buddy_of(left_buddy, source_order);

        remove_block(left_buddy, source_order);
        remove_block(right_buddy, source_order);

        if (is_correct_alignment_for_order(left_buddy, source_order+1))
            return insert_block(left_buddy, source_order+1);
        else
            return insert_block(right_buddy, source_order+1);
    }
...
public:
    ...
    PageDescriptor *alloc_pages(int order) override
    {
        ...
        //ensure the order is in correct range
        if (order >= MAX_ORDER)
            return NULL;

        PageDescriptor *pgd = _free_areas[order];

        //if there is free space in order-16's list, just return it
        if (_free_areas[order]) {
            ...
            for (int i  = order; i < MAX_ORDER; i++) {
                if (_free_areas[i]) {
                    PageDescriptor *block_pointer = _free_areas[i];

                    //Keep splitting down to given order
                    while (i > order) {
                        block_pointer = split_block(&block_pointer, i);
                        i--;
                    }
                    //remove as it's been allocated
    ...
    void free_pages(PageDescriptor *pgd, int order) override
    {
        ...
        //we place block inside corresponding free areas list
        PageDescriptor **slot = insert_block(pgd, order);

        //coalescing buddies back up to the maximum order
        while (order < MAX_ORDER) {
            PageDescriptor *buddy = buddy_of(*slot, order);
            PageDescriptor *block_pointer = _free_areas[order];
            bool merged = false;

            //iterate through free list of order and check if buddy exists there
            while (block_pointer) {
    ...
    {
        //not_implemented();
		PageDescriptor *find_block;
		bool found = false;

		for (int order = 0; order < MAX_ORDER && found == false; order++) {
		    PageDescriptor *block_pointer = _free_areas[order];

		    while (block_pointer && found == false) {
		        if (pgd >= block_pointer && pgd < (block_pointer + pages_per_block(order))) {
                    find_block = block_pointer;

                    //Split until only the page being reserved is allocated
		            for (int i = order; i > 0;i--) {
		                PageDescriptor *left_buddy = split_block(&find_block, i);
		                PageDescriptor *right_buddy = buddy_of(left_buddy, i-1);

		                //check which one pgd is in
		                if (pgd >= right_buddy) {
		                    find_block = right_buddy;
		                }else{
		                    find_block = left_buddy;
		                }
		            ...
		            //Might be in another block in the same order
		            block_pointer = block_pointer -> next_free;
		        }
		    }
		}

		//Check for pgd in the list in order zero and remove so it can be reserved
		PageDescriptor *order_zero = _free_areas[0];
		while (order_zero) {
		    if(pgd == order_zero) {
		        remove_block(pgd, 0);
		        return true;
		    } else {
		        order_zero = order_zero -> next_free;
		    }
		}
		return false;
    }
    ...
    bool init(PageDescriptor *page_descriptors, uint64_t nr_page_descriptors) override
    {
        ...
        int nr_blocks = nr_page_descriptors/pages_per_block(MAX_ORDER-1);

        PageDescriptor *base = page_descriptors;
        _free_areas[MAX_ORDER-1] = base;

        for(int i = 0; i < nr_blocks; i++) {

            base -> next_free = base  + pages_per_block(MAX_ORDER-1);
            base = base + pages_per_block(MAX_ORDER-1);
